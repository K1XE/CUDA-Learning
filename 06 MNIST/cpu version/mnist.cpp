...
void load_data(const char *filename, float *data, uint size)
{
...
void load_labels(const char *filename, uint *labels, uint size)
{
    FILE *file = fopen(filename, "rb");
    if (file == NULL)
    {
    ...
    size_t read_size = fread(labels, sizeof(uint), size, file);
    if (read_size != size)
    {
        fprintf(stderr, "读取数据异常: 应有 %d 元素, 实有 %zu 元素\n", size, read_size);
        exit(1);
...
// 定义交叉熵为LOSS
float cross_entropy_loss(float *output, uint *labels, uint bsz)
{
...
// 清空累计梯度
void zero_grad(float *grad, uint size)
{
    memset(grad, 0, size * sizeof(float));
}

// ReLU 反向传播
void relu_backward(float *grad, float *mat, uint size)
{
    for (int i = 0; i < size; i++)
    {
        grad[i] *= (mat[i] > 0);
...
// Bias 反向传播
void bias_backward(float *grad_bias, float *grad, uint bsz, uint size)
{
    for (int i = 0; i < size; i++)
    {
        grad_bias[i] = 0.0f;
...
// 计算输出层梯度
void compute_output_gradients(float *grad_output, float *output, uint *labels, uint bsz)
{
    for (int b = 0; b < bsz; b++)
    {
...
void update_gradients(
    ...
    uint bsz,            // 小批量样本数
    uint prev_size,      // 前一层神经元个数（输入维度）
    uint curr_size       // 当前层神经元个数（输出维度）
)
{
...
void train(simpleNeuralNetwork *nn, float *X_train, uint *y_train)
{
    // 1. 为前向传播中间结果分配缓冲
...
int main(int argc, char **argv)
{
    ...
    uint *y_train = (uint *)malloc(TRAIN_SIZE * sizeof(uint));
    float *X_test = (float *)malloc(TEST_SIZE * INPUT_SIZE * sizeof(float));
    uint *y_test = (uint *)malloc(TEST_SIZE * sizeof(uint));
